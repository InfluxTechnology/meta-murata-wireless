...
function switch_to_cypress_sdio() {
  ...
  if [ $cyw_module == "2EA-SDIO" ]; then
     fw_setenv fdt_file imx8mn-influx-rex-smart_${DTB_VER}-2.dtb 2>/dev/null
     fw_setenv bt_hint cypress_2ea
     fw_setenv cmd_custom
     move_ko
  else
     fw_setenv fdt_file imx8mn-influx-rex-smart_${DTB_VER}.dtb 2>/dev/null
...
function switch_to_cypress_pcie() {
  ...
  if [ $cyw_module == "2EA-PCIE" ]; then
     fw_setenv fdt_file imx8mn-influx-rex-smart_${DTB_VER}-pcie-2.dtb 2>/dev/null
     fw_setenv bt_hint cypress_2ea
     fw_setenv cmd_custom
     move_ko
  else
     fw_setenv fdt_file imx8mn-influx-rex-smart_${DTB_VER}-pcie.dtb 2>/dev/null
     fw_setenv bt_hint cypress
     fw_setenv cmd_custom
     restore_ko
  fi
